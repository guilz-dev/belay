...
export async function formatOperationalInsights(
  ...
  if (insights.oq3Spike) {
    lines.push(
      `OQ3 spike: ${insights.oq3Spike.ok ? 'ok' : 'failed'} at ${insights.oq3Spike.path}${
        insights.oq3Spike.recordedAt ? ` (${insights.oq3Spike.recordedAt})` : ''
      }`,
    )
