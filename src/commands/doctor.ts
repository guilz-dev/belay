...
export async function doctorProject(options: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  if (!existsSync(configPath)) {
    ...
    try {
      ...
      notes.push(
        'Verdict engine: v2 (location × opacity × effect × confidence). Shell gates use the v2 classifier; audit records include schemaVersion 2 axes when available.',
      )
  ...
  if (loadedConfig) {
    ...
    if (loadedConfig.egress.enabled) {
      ...
      notes.push(
        `Egress proxy: enabled — external-effect demotion active only while proxy runs (demoteL3External=${loadedConfig.egress.demoteL3External}), listen ${egress.host}:${egress.port}.`,
      )
      if (!egress.running) {
        warnings.push(
          'Egress is enabled in config but the local proxy is not running. Run agent-belay egress start.',
        )
      } else {
        notes.push(`Egress proxy running (pid ${egress.pid}).`)
        if (egress.foreignProxy) {
          warnings.push(
            `Egress listen port ${egress.host}:${egress.port} is occupied by another proxy${egress.boundRepoRoot ? ` for ${egress.boundRepoRoot}` : ''}. Do not use agent-belay egress env for this repository.`,
          )
