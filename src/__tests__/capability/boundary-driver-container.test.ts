...
describe('container boundary driver', () => {
  it('probes docker when available', async () => {
  ...
  it('runs a command in an isolated container', async () => {
    if (!(await isDockerAvailable())) {
      return
    }
    const driver = createContainerBoundaryDriver()
  ...
  it('blocks writes on read-only mounts and allows them on read-write mounts', async () => {
    if (!(await isDockerAvailable())) {
      return
    }
    const driver = createContainerBoundaryDriver()
  ...
  it('routes network through egress proxy env when configured', async () => {
    const driver = createContainerBoundaryDriver({
      egressProxyEnv: {
