...
const SCOPE_BLAST_RADIUS: Record<BlastRadiusScope, string> = {
  none: 'none',
  file: 'single file',
  dir: 'directory tree',
...
export function blastRadiusLabel(scope: BlastRadiusScope): string {
  return SCOPE_BLAST_RADIUS[scope]
}

export function computeAssessmentFromAttributes(attributes: ShellAttributes): Assessment {
  ...
  if (
    attributes.isFlaggedKey ||
    attributes.redirectKind === 'truncate' ||
    attributes.redirectKind === 'append'
  ) {
    const reversibility =
      attributes.flags.includes('-rf') || attributes.flags.includes('-fr')
        ? 'irreversible'
        : attributes.redirectKind === 'append'
...
export function calibrateConfidence(attributes: ShellAttributes, base: number): number {
  let confidence = base
...
export function mergeAgentAssessment(
  ...
  agent?: Assessment,
): { assessment: Assessment; mismatch: boolean } {
  if (!agent) {
    return { assessment: independent, mismatch: false }
  }

  const mismatch =
    (agent.external === false && independent.external === true) ||
