...
export const SHOW_DONT_RUN_LEAD =
  'These steps may help undo the observed effect — confirm each step before running:'

const IRREVERSIBLE_REASON_PREFIXES = ['tier0_'] as const
...
export function containsDeniedRecoveryPattern(text: string): boolean {
  return DENIED_RECOVERY_PATTERNS.some((pattern) => pattern.test(text))
}

function isIrreversibleTarget(target: RecoverTargetInput): boolean {
...
function extractPathHints(summary: string): string[] {
...
function localMutationAdvice(target: RecoverTargetInput, git?: GitProbeResult): string[] {
  const advice: string[] = []
