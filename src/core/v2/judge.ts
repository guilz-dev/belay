...
export function createDeterministicJudgeStub(): Tier1Judge {
  return {
    evaluate() {
...
export function createFailClosedJudge(): Tier1Judge {
  return {
    async evaluate() {
      return {
...
export function createOllamaJudge(
  model = 'gemma4:e2b',
  ...
  return {
    async evaluate(input) {
      ...
      try {
        ...
        const parsed = JSON.parse(payload.response ?? '{}') as Partial<Tier1Verdict>
        return {
