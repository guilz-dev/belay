...
function findDangerousFlags(tokens) {
    return tokens.some((token) => FIND_DANGEROUS.has(token) || token.startsWith('-exec') || token.startsWith('-ok'));
}
...
export function analyzeShellSegment(params) {
    ...
    const hasCredentialHeader = segmentTokens.some((token) => token === '-H' || token === '--header' || /authorization/i.test(token));
    const findDangerous = key === 'find' && findDangerousFlags(segmentTokens);
    const isCustomAllow = (options.customAllowCommands ?? []).some((pattern) => matchesCustomCommand(normalizedCommand, key, pattern));
    const isCustomExternal = (options.customExternalCommands ?? []).some((pattern) => matchesCustomCommand(normalizedCommand, key, pattern));
    if (isUnparseable) {
...
export function matchesPolicyRule(match, attributes) {
