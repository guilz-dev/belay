...
function runnerCommand(platform, repoRoot, hookName, ...args) {
  const hooksDir = path3.join(path3.resolve(repoRoot), ".claude", "hooks");
  return buildRunnerInvocation(platform, hooksDir, repoRoot, hookName, ...args);
}
var claudeLayout = {
  name: "claude",
  configPath(repoRoot) {
    return path3.join(repoRoot, ".claude", "belay.config.json");
  },
  hooksSettingsPath(repoRoot) {
    return path3.join(repoRoot, ".claude", "settings.json");
  },
  hooksDir(repoRoot) {
    return path3.join(repoRoot, ".claude", "hooks");
  },
  runtimeDir(repoRoot) {
    return path3.join(repoRoot, ".claude", "belay", "runtime");
  },
  repoLocalStateDir(repoRoot) {
    return path3.join(repoRoot, ".claude", "belay");
  },
  defaultAuditLogPath(_repoRoot) {
    return path3.join(".claude", "belay", "audit.ndjson");
  },
