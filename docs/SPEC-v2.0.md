...
- `allow` の正直さを壊さず、confidence を `verified_substrate` に上げる
  追加層である
- opaque routine class(`npm run`, `make`)の**原理的な本筋の解**は将来の
  L2 observed execution だが、v2.0 では approval cache で摩擦を 1 回化する
...
> 過剰検出**した。2 軸を 1 boolean に詰めると 2B は両軸とも崩す。分離した形で
> curated corpus 35/35。詳細は WS-D の R13/R14 と spike trace。
...
>
> Both default to `true` when unsure. Changes confined inside the current repo,
> and running tests / building locally, are NEITHER.

`external_change` または `destroys_outside_repo` のいずれかが `true` なら `ask`。
...
4. Tier1 は列挙不能な領域(任意の DB クライアント / クラウド CLI / inline code)を
   2 boolean で補う。`external_change` は DB・ネットワーク・remote の stateful
...
コード内 path による repo 外破壊を拾うために使う。
...
### R13 — corpus は新 contract の両端を叩く

v2.0 corpus には少なくとも次を固定する:

- chain / subshell / substitution / parse failure
- wrapper / inline interpreter / opaque launcher
- repo 外ローカル mutation
- `.git` / `sensitivePaths` 破壊
- cwd 欠落 / multi-root ambiguity / symlink escape
- secret-bearing commands
- routine launcher の approval-cache hit/miss

特に次を明示的に含める:

- `npm test` / `npm run build` が毎回 ask にならないこと
- `rm -rf .git` が `allow` にならないこと

### R14 — spike-first validation を出荷手順に含める

core 統合前に、`~/.belay-spike/verdict.mjs` 相当の spike 実装へ同 contract を入れ、
curated corpus を通す。文書先行で contract を凍らせない。

v2.0 の統合経路:

- `src/core/classify-shell.ts` → 新 verdict core
- `src/core/shell-tokenizer.ts` / `shell-substitution.ts` → 構造解析器
- `src/core/judgment.ts` / `model-assist.ts` → repo 外/外部 change 問い
- `src/adapters/shared/gate-runtime.ts` → trace schema と approval cache 合成
...
- T9: spike verdict が curated corpus を通り、その後 core integration でも
  同じ corpus が green
