...
.belay/manifests/<basename>.json             control-plane trust record
        ...
        +--------------------+-----------------------------+
                    ...
                    TrustedManifestLoader
                             |
              ShellFrontendRouter (legacy | shadow | canary | mvdan)
                             |
         +-------------------+-------------------+
         |                                       |
   per-frontend SemanticEffectLowerer     (same for each invoked frontend)
         |                                       |
         v                                       v
 built-in semantic decoder              built-in semantic decoder
         |                                       |
 process.grammar_unknown? (isGrammarUnknownOnly) |
         | no                              | yes |
         v                                   v     v
 built-in requirements              ManifestRuleMatcher (per segment)
         |                                   |
         +-------------------+----------------+
                             v
              EffectPlan v1 (per frontend; compare only after manifest)
                             |
         shadow / canary: authorization projection compare, then canary disagreement
                             v
                      canonical EffectPlan v1
                             v
                        PolicyEngine
```

Each rollout mode invokes one or two shell frontends before semantic lowering. Manifest
resolution always runs **inside** each frontend's lowering path **after** built-in decoders and
**before** cross-frontend EffectPlan comparison. Manifest matching uses the same
`isGrammarUnknownOnly` gate as argv delegate peeling in
`src/core/effect-ir/shell-lower/argv-delegate-gate.ts`: only a segment whose process requirements
are exactly the spawn + indeterminate `process.grammar_unknown` pair is eligible.
...
This section aligns effect manifests with
[`2026-09-19-mvdan-shell-frontend-migration-design.md`](2026-09-19-mvdan-shell-frontend-migration-design.md)
rollout modes. EffectPlan comparison across frontends happens only **after** each invoked frontend
has completed built-in semantic lowering **and** per-segment manifest resolution.

| Mode | Frontends lowered | Manifest on canonical path | Manifest on candidate path | Comparison |
| --- | --- | --- | --- | --- |
| `legacy` | legacy only | applied to canonical plan | n/a | n/a |
| `mvdan` | mvdan only | applied to canonical plan | n/a | n/a |
| `shadow` | legacy + mvdan | applied to legacy canonical plan | match/audit telemetry only; **must not** change canonical plan, permissions, or hook response | syntax telemetry only (existing shadow rules) |
| `canary` | mvdan + legacy | applied to **both** plans before compare | same as mvdan row | authorization projection compare on manifest-adjusted plans; mismatches append `indeterminate` + `parser.disagreement` **after** comparison |

Rules:

1. **Per-frontend lowering.** Each frontend invocation runs the same semantic lowering pipeline
   (built-in decoders, `isGrammarUnknownOnly` gate, optional manifest replacement, argv delegate
   peeling). Do not apply manifests only on the post-compare canonical plan.
2. **Shadow candidate isolation.** When `shellFrontendMode` is `shadow`, manifest outcomes on the
   mvdan candidate plan are recorded in audit/explain telemetry only. They cannot replace, remove, or
   add requirements on the legacy canonical plan.
3. **Canary ordering.** When `shellFrontendMode` is `canary`, build legacy and mvdan EffectPlans
   with manifests applied on each path, compare normalized authorization projections, then apply
   `parser.disagreement` handling to the mvdan canonical plan. Never union requirements from the two
   frontends and never choose the more permissive plan.
4. **Non-replaceable indeterminate.** Manifest matching must not remove or replace requirements
   whose evidence includes `parser.disagreement`, parser or frontend failure signals (for example
   `parser.artifact_unavailable`), or `indeterminate` requirements introduced because shell syntax
   projection is `partial` / incomplete. Manifests may replace only the paired
   `process.grammar_unknown` spawn + indeterminate requirements on **complete** segments that pass
   `isGrammarUnknownOnly`.
5. **Fail closed.** Manifest load, identity, trust, or matcher failures leave the original
   `process.grammar_unknown` pair in place on that lowering path; they never synthesize allow.
...
### Shell frontend rollout modes

- `legacy`: a trusted manifest on an unknown command lowers the canonical plan; shadow/candidate
  paths are not invoked.
- `shadow`: legacy canonical plan includes manifest lowering; mvdan candidate manifest outcomes appear
  only in audit telemetry and do not change the canonical plan or hook decision.
- `canary`: legacy and mvdan plans both apply manifest lowering before authorization projection
  compare; `parser.disagreement` is appended only after that compare when projections differ.
- `mvdan`: manifest applies on the mvdan canonical path only.
- `parser.disagreement`, frontend failure signals, and partial-syntax `indeterminate` requirements
  are never removed or replaced by manifest matching in any mode.
