...
- **重要な前提の訂正（レビュー指摘を受けて）**: 現状 `runTransactionalExecution` → `runShellCommand`（[git-worktree.ts:96](../src/core/transactional/git-worktree.ts#L96)）は `spawn(command, { shell: true, env: process.env })` で**ホスト上に直接**実行しており、`broker.ts` の判定（`isCapabilityBrokerDemotionActive`）はこの経路に一切配線されていない（分類器のFS-scope判定にのみ使用）。つまり現状の「サンドボックス」はgit worktreeによる**ファイル変更の隔離**であり、プロセス実行そのものの隔離（コンテナ/制限環境）ではない。この提案が導入するcapability tokenは**その隔離を作るものではなく**、「どの経路がどの境界を通ったか」を型と実行時チェックで可視化・誤用防止するものに過ぎない。実境界の実装は別課題として明記する（§5, §6）。
- 実際に手を動かせる転用先は2つ：
