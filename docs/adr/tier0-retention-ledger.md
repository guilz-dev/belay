...
| prescan `sensitive_path_mutation` / `persistent_agent_path` | Redirect/write to secrets or agent startup files (`src/core/verdict/persistent-paths.ts`) | `cat .env` | `echo x >> .env`, `echo x >> ~/.zshrc`, `echo x >> ~/.ssh/authorized_keys` |
...
| repo-outside secret/credential paths | secret overwrite/exfil |
