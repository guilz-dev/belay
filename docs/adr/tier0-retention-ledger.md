...
|---|---|---|---|
| `control_plane_mutation` | Mutating Belay hook/config/audit can disable the floor itself | `Read .cursor/belay.config.json`, `Read .git/HEAD` | `tee` into `.cursor/belay.config.json` |
...
| `high_stakes_path` (action-aware) | Destructive mutation on `.git` / sensitive paths; **reads** on control-plane and git metadata paths are allow | `cat .env`, `Read .git/HEAD` | `rm -rf .git` |
...
| `rm .cursor/hooks/belay-runner` | control-plane mutation |
| `Write .cursor/belay/runtime/core.mjs` | control-plane mutation |
...
| `Read .cursor/belay.config.json` / `Read .cursor/belay/audit.ndjson` | control-plane introspection (read-only) |
| `stat .cursor/hooks/belay-runner` / `head .cursor/belay/audit.ndjson` | control-plane introspection (read-only) |
| `Read .git/HEAD` / `Read .git/config` | git metadata read (not mutation) |
