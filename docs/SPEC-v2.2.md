...
| **WS-Skill** | skill フロントドア（R-S1〜S5） | **実装済み**（残件: G-B1 実機記録のみ） | G-A クローズ。G-B1 Decision 確定 / Verification は [`gates/G-B1-cursor-skill-ux.md`](gates/G-B1-cursor-skill-ux.md) |
| **WS-Pkg** | 配布 packaging（plugin/hook-package/skill） | 要件化（R-X4） | WS-Codex/WS-Skill 実装済み |

**ゲート定義:**
- **G-A（決定・人間）**: ✅ **クローズ**。O1=`true`（明示のみ）確定 / O5=R-S5 scope 軸で解決 /
  v2.2 スコープ＝WS-Skill 全部入れ（R-S1〜S5）で確定。
- **G-B1（Cursor 実機・Decision 確定 / Verification pending）**: skill の明示呼び出し UX
  （commands vs skills）の方針は確定済み（§2.1、[`gates/G-B1-cursor-skill-ux.md`](gates/G-B1-cursor-skill-ux.md)）。
  実機記録（Execution/Result）は gate 文書に未記入。
...
> **WS-Codex** と **WS-Skill** は実装済み。残る WS-Skill 作業は G-B1 実機記録の記入のみ
> （機能未実装ではない）。
...
belay-as-skill が満たす規範。**R-S1〜S5 は実装済み**（`init-wizard`、`--scope`、`slash commands`、
`skill-quality` テスト）。G-B1 の実機記録のみ gate 文書で pending。
...
### R-S3 — 対話インタフェース（SHOULD・**実装済み**）
skill は**判定をしない**。`/belay …` を受けて `agent-belay` CLI を叩かせる薄い
オーケストレーション層に徹する（判定は常に core）。

| ユーザー向け | エージェントが実行する CLI | 目的 | CLI 現状 |
| --- | --- | --- | --- |
| `/belay why <command>` | `agent-belay explain --command "..."` | verdict（location/opacity/effect/permission、reason、Tier0/Tier1、judgeTrace）の人間可読説明 | `explain --command` 実装済み（T22） |
...
- **ゲート**: 明示起動（slash）か description 自動発火かの設計は G-B1（§2.1）で確定。
  `disable-model-invocation: true` + slash commands を採用（gate 文書参照）。
...
| フィールド | 用途 | 決定 |
| --- | --- | --- |
| `disable-model-invocation` | `true` で `/belay` 明示時のみ起動 | **`true`（明示のみ）に確定**。G-B1 で `true` 維持を確定（`false` への再評価は行わない） |
...
- **実装含意**: `project` / `global` は [`docs/design/global-scope.md`](design/global-scope.md) の
  path matrix で実装済み。`managed` は Codex のみ・未実装（`--scope managed` は明示エラー）。
- **受け入れ基準**: T29（`--scope project|global` が provider 別の正しい場所へ skill+hook を書く／
  既定が project／`managed` は Codex のみ・sudo 要件を明示）。

### 2.1 G-B1 — Cursor commands/skills UX 検証
Cursor における skill の**明示呼び出し UX**（slash 的な明示起動か、description 一致による
自動発火か、その併用か）を実機で確認するゲート。**Decision は確定済み**（`disable-model-invocation:
true`、slash commands、deny message 一次導線）。**Verification**（Execution/Result）は
[`gates/G-B1-cursor-skill-ux.md`](gates/G-B1-cursor-skill-ux.md) に未記入。
...
- **G（着手条件）**: WS-Codex と WS-Skill は実装済み。WS-Pkg は要件化段階。
...
## 6. skill 品質チェックリスト（v2.2 出荷ゲート）

WS-Skill 実装時に満たした性質。未チェックは skill 本文外または別 WS の残件。

- [x] `description` に WHAT（belay 承認・ゲート補助）と WHEN（denied, blocked, high-risk,
      belay-approve）を含む（R-S4 / T23）
- [x] hook 未導入時に `npx agent-belay init` を促す手順がある（R-S2 / 安全境界 §1.1）
- [x] `agent-belay doctor` で導入確認する手順がある
- [x] `/belay-approve` フローが明確
- [x] WS-J コマンド（why / explain / status）の手順と出力テンプレート（R-S3）
- [ ] cloud judge 利用時の `--accept-cloud-judge` 等 v2.1.1 契約への言及（SKILL 本文外。
      `agent-belay status` / README で扱う）
- [x] マルチ adapter 向け init コマンドの分岐（Cursor / Claude / Codex）— `init-wizard` / `--adapter`
- [x] `disable-model-invocation` の方針を決め frontmatter に明記（O1 / G-B1 Decision）
- [x] `npx skills add` 用に `skills/belay/` がパッケージに同梱されている
- [x] skill 経由の手順が conformance / CLI テストで壊れていないこと（T23 スナップショット）
- [x] **README / marketplace 説明の安全境界を退行させない**: skill-only=助言モード・
      enforcement は `init` 必須を保持（§1.1）
- [x] **README が多ホスト導入を反映**: `init --adapter` / `init-wizard` / `--scope` 導線。
      v2.x 詳細は [ROADMAP.md](ROADMAP.md) 参照（README のバージョン表は v0.x 中心のまま）
...
v2.1/v2.1.1 と同じ粒度の受け入れゲート。WS-Codex 分（T24〜T27）と WS-Skill 分（T20〜T23、T29）は
実装済み。WS-Pkg 分（T28）は未着手。

| # | 対象 | 検証内容 | 状態 |
| --- | --- | --- | --- |
| T20 | R-S1 安全境界 | skill 経路に判定ロジックが無く、判定が単一コア経由であること | ✅ 実装済み |
| T21 | R-S2 advisory | hook 未導入リポジトリで skill 手順が advisory 警告 + `init` 導線を返す（skill-only=floor でない負テスト） | ✅ 実装済み |
| T22 | R-S3 CLI | `agent-belay explain --command "..."` が verdict 要素（location/effect/permission/reason）を出力 | ✅ 実装済み |
| T23 | R-S4 skill 品質 | SKILL.md スナップショット + `description` トリガー語の存在 | ✅ 実装済み |
...
| T29 | R-S5 scope | `--scope project\|global` が provider 別の正しい場所へ skill+hook を書く／既定 project／`managed` は Codex のみ | ✅ 実装済み |

## 8. オープンクエスチョン（ゲート対応）

| # | 論点 | 確定方法 / ゲート |
| --- | --- | --- |
| O1 | `disable-model-invocation` の方針 | ✅ **解決（`true`=明示のみ）**。G-B1 で `true` 維持確定 |
| O2 | Cursor の skill 明示呼び出し UX | ✅ **Decision 解決**（`disable-model-invocation: true` + slash commands）。**Verification pending**（G-B1 gate 文書の Execution/Result） |
